...
class ControleManualCancela(models.Model):
    ...
    vagas_dispo_moto = fields.Integer(related='morador_id.dispo_vagas_moto',
                                      store=True)
    vagas_dispo_bicicletario = fields.Integer(related='morador_id.dispo_vagas_bicicletario',
                                       store=True)
    ...
    @api.one
    @api.constrains('vagas_dispo_bicicletarios')
    def _check_vaga_dispo_bicicletarios(self):
        dispo_vagas_bicicletarios = 0
        if not self.overwrite:
            if self.sentido == 'in':
                if self.tipo_manual == 'bicicletarios':
                    dispo_vagas_bicicletarios = self.vagas_dispo_bicicletarios
                    if dispo_vagas_bicicletarios < 0:
                        raise ValidationError(
                            "Morador não possui vagas de bicicletas disponíveis")
    ...
    def write(self, vals):
        ...
        if not self.overwrite:
            ...
            if self.sentido == 'out':
                ...
                if self.tipo_manual == 'bicicletas':
                    dispo_vagas_bicicletas = self.vagas_dispo_bicicletas + 1
                    if dispo_vagas_bicicletas <= self.morador_id.total_vagas_bicicletas:
                        self.morador_id.dispo_vagas_bicicletas = dispo_vagas_bicicletas
                    else:
                        self.morador_id.dispo_vagas_bicicletas =\
                            self.morador_id.total_vagas_bicicletas
        sql_write = """
...
class ControleManualPrestServ(models.Model):
    ...
    class ControleManualPets(models.Model):
    _name = 'occ.controle.manual.pets'
    _rec_name = 'pets_id'
    _description = u'Tabela com os dados parara abertura do portão para pets'
    _table = 'occ_controle_manual_pets'
    _order = 'create_date desc'
    sentido = fields.Selection([('in', u'Entrada'), ('out', u'Saída')],
                               u"Sentido", required=True, default='in')
    bloco_id = fields.Many2one('occ.bloco', u'Bloco', required=True)
    apto_id = fields.Many2one('occ.pets';, u'Apartamento', required=True,
                              domain="[('bloco_id','=',bloco_id)]")
    funcionario_id = fields.Many2one('occ.pets',
                                     u'Pets',
                                     required=True)
    morador_id = fields.Many2one('occ.morador', u'Entrada autorizada por',
                                 domain="[('apto_id','=',apto_id)]",
...
class ControleManualPrestServ(models.Model):
    _name = 'occ.controle.manual.bicicletarios'
    _rec_name = 'bicicletarios_id'
    _description = u'Tabela com os dados para abertura portão empresas'
    _table = 'occ_controle_manual_bicicletarios'
    _order = 'create_date desc'
    sentido = fields.Selection([('in', u'Entrada'), ('out', u'Saída')],
                               u"Sentido", required=True, default='in')
    bloco_id = fields.Many2one('occ.bloco', u'Bloco', required=True)
    apto_id = fields.Many2one('occ.bicicletarios', u'Apartamento', required=True,
                              domain="[('bloco_id','=',bloco_id)]")
    funcionario_id = fields.Many2one('occ.bicicletarios',
                                     u'Moradores',
                                     required=True)
    morador_id = fields.Many2one('occ.morador', u'Entrada autorizada por',
                                 domain="[('apto_id','=',apto_id)]",
